...
class ShikimoriRepositoryImpl @Inject constructor(
    ...
    override fun getScreenshots(id: Int): Flow<NetworkResult<List<ShikimoriScreenshot>>> = flow {
        emit(NetworkResult.Loading)
        try {
            delay(throttleMs)
            val response = api.getScreenshots(id)
            emit(
                NetworkResult.Success(
                    response.mapNotNull { dto ->
                        // Путь у Shikimori относительный — без домена картинка
                        // просто не загрузится.
                        val preview = dto.preview?.toShikimoriUrl()
                        val original = dto.original?.toShikimoriUrl()
                        if (preview == null && original == null) null
                        else ShikimoriScreenshot(preview = preview, original = original)
                    }
                )
            )
        } catch (e: Exception) {
    ...
    override fun searchCharacters(query: String, limit: Int): Flow<NetworkResult<List<ShikimoriCharacterSearchResult>>> = flow {
        emit(NetworkResult.Loading)
        try {
            if (query.isBlank()) {
            ...
            delay(throttleMs)
            val response = api.searchCharacters(query, limit)
            emit(
                NetworkResult.Success(
                    response.map { dto ->
                        ShikimoriCharacterSearchResult(
                            id = dto.id,
                            name = dto.name,
                            russian = dto.russian,
                            imageUrl = (dto.image?.preview ?: dto.image?.x96).toShikimoriUrl(),
                        )
                    }
                )
            )
    ...
    /**
     * Shikimori отдаёт относительные пути вида `/system/screenshots/…`.
     * Без домена такая ссылка не грузится ничем.
     */
