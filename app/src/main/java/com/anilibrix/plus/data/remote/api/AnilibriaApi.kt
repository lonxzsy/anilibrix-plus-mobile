...
interface AnilibriaApi {
    ...
    suspend fun getTimecodes(): List<ReleaseDto>

    @POST("accounts/users/me/views/timecodes")
    suspend fun updateTimecode(@Body request: TimecodeRequest)

    @DELETE("accounts/users/me/views/timecodes")
    suspend fun deleteTimecode(@Query("title_id") titleId: Long, @Query("episode_id") episodeId: Long)
}
