        ...
        try {
            ...
            val days = response
                ...
                .map { (dayValue, items) ->
                    ...
                    val publishedEpisode = items.firstOrNull()?.publishedReleaseEpisode?.ordinal?.toInt()
                    val nextEpisode = items.firstOrNull()?.nextReleaseEpisodeNumber
                    val releases = items.mapNotNull { it.release?.toDomain() }
                    ScheduleDay(
                        day = dayName,
                        releases = releases,
