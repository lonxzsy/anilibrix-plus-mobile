...
class JikanRepositoryImpl @Inject constructor(
    ...
    override fun getCharacter(characterId: Long): Flow<NetworkResult<MalCharacterDetail>> = flow {
        emit(NetworkResult.Loading)
        try {
            delay(throttleMs)
            val character = api.getCharacter(characterId).data
            if (character == null) {
                emit(NetworkResult.Error("Персонаж не найден"))
            } else {
                emit(NetworkResult.Success(character.toDomain()))
            }
        } catch (e: Exception) {
            emit(NetworkResult.Error(e.message ?: "Unknown error", e))
        }
    ...
    override fun getRecommendations(malId: Long): Flow<NetworkResult<List<MalAnime>>> = flow {
        ...
        try {
            ...
            val list = response.data?.mapNotNull { it.entry?.firstOrNull()?.toDomain() } ?: emptyList()
            emit(NetworkResult.Success(list))
        } catch (e: Exception) {
            emit(NetworkResult.Error(e.message ?: "Unknown error", e))
        }
    ...
    private fun com.anilibrix.plus.data.remote.dto.JikanCharacterFullDto.toDomain(): MalCharacterDetail =
        MalCharacterDetail(
            malId = malId,
            name = name,
            ...
            favorites = favorites,
            imageUrl = images?.jpg?.imageUrl,
            voiceActors = voices.mapNotNull { actor ->
            ...
            appearsIn = anime.mapNotNull { entry ->
                entry.anime?.let {
                    CharacterAppearance(
                        malId = it.malId,
                        title = it.title,
